    ...
    if ( mode == 1 ) {
		...
		if ( ! p_->empty() ) {
			...
			for ( i=fr; i<=to; i++ ) {
				...
				p_->findByName( idx )->explore( this );
			}
			indent_=0;
		} else {
			p_->explore( this );
...
int         type;
	...
	if ( ! p->tail()->isHub() ) {
        ...
        printf(" \"%s,%s\"", regInterfaceTypeNames[ifType], regTypeNames[rgType]);
	}
    printf("\n");
    indent_ += 2;
	return true;
...
void
YCPSWASYN::visitPost(ConstPath p)
{
	indent_ -= 2;
}

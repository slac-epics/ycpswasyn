...
YCPSWASYN::YCPSWASYN(const char *portName, Path p, const char *recordPrefix, int recordNameLenMax, int autogenerate, const char* dictionary)
    : asynPortDriver(
        portName,
        MAX_SIGNALS,
        NUM_PARAMS,
    ...
    recordCount(0)
{
    ...
    if (autogenerate)
        autogenerateDatabase(autogenerate);
...
//                                                                                 //
/////////////////////////////////////////////////////////////////////////////////////
int YCPSWASYN::autogenerateDatabase(int mode)
{
    // Create file names for the register and PV list dumps
    std::string pre                  = DUMP_FILE_PATH + std::string(this->portName_) + "_" + string(recordPrefix_) + "_";
    ...
    if ( mode == 1 ) {
		...
		try {
			...
			if ( ! p_->empty() ) {
				// Generate a flat database with all non-leaf arrays expanded
				unsigned fr = p_->getTailFrom();
				unsigned to = p_->getTailTo();
				unsigned i;
				Child    tl = p_->up();
				char     idx[200];
				std::string prefixPath("root"+p_->toString());
				const char *p1, *p2;
				p1 = prefixPath.c_str();

				while ( (p2 = strchr(p1, '/')) ) {
					fprintf( regDumpYamlFile.f(), "%*s%.*s:\n", regDumpYamlFile.indent(), "", (int)(p2 - p1), p1);
					regDumpYamlFile.pushIndent();
					p1 = p2 + 1;
				}
				fprintf( regDumpYamlFile.f(), "%*s%s:\n", regDumpYamlFile.indent(), "", p1);
				regDumpYamlFile.pushIndent();
				for ( i=fr; i<=to; i++ ) {
					snprintf(idx, sizeof(idx), "%s[%d]", tl->getName(), i);
					p_->findByName( idx )->explore( &regDumpYamlFile );
				}
			} else {
				p_->explore( &regDumpYamlFile );
			}
		} catch (CPSWError &e) {
		}
	}
...
extern "C" int YCPSWASYNConfig(const char *portName, const char *yaml_doc, const char *rootPath, const char *ipAddr, const char *recordPrefix, unsigned int recordNameLenMax, int autogeneration, const char* dictionary)
{
    ...
    new YCPSWASYN(portName, p, recordPrefix, recordNameLenMax, autogeneration, dictionary);

...
int YCPSWASYN::LoadRecord(int regType, const recordParams& rp, const string& dbParams, Path p)
{
    ...
    pvDumpFile->write("%s%*s", rp.recName.c_str(), recordNameLenMax_ - rp.recName.size() + 4, "");
...
int YCPSWASYN::autogenerateDatabase()
{
    ...
    if (autogenerationMode_ == 1)
    {
        ...
        printf("Opening file \"%s\" for dumping not found keys.\n", keysNotFoundFileName.c_str());
        keysNotFoundFile = new YCPSWASYNRAIIFile(keysNotFoundFileName, "w");
    }

...
int YCPSWASYN::autogenerateDatabase()
{
    ...
    try
    {
        YCPSWASYNRegDumpYamlFile regDumpYamlFile( pre, this );

        fprintf( regDumpYamlFile.yamlFile.f(), "recordPrefix: %s\nroot:\n", recordPrefix_.c_str() );
    ...
    // Print counters
    printf("Total register processed = %ld\n", recordCount);
    printf("  - RO registers         = %ld\n", nRO - nRW);
    printf("  - RW registers         = %ld\n", nRW);
    printf("  - Command registers    = %ld\n", nCMD);
    printf("  - Stream registers     = %ld\n", nSTM);
    printf("\n");
...
YCPSWASYNRegDumpYamlFile::YCPSWASYNRegDumpYamlFile(const std::string &pre, YCPSWASYN *drv)
    :
    drv_   ( drv ),
    indent_( 0   ),
...
bool YCPSWASYNRegDumpYamlFile::visitPre(ConstPath p)
{
    ...
    fprintf( yamlFile.f(), "%*s%s:", indent(), "", lst);
    if ( ! p->tail()->isHub() )
    {
        ...
        fprintf( yamlFile.f(), " \"%s,%s\"", regInterfaceTypeNames[ifType], regTypeNames[rgType]);
        fprintf( textFile.f(), "%s\n", p->toString().c_str());
    }
    fprintf( yamlFile.f(), "\n");
    pushIndent();
    return true;
